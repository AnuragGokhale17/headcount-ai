...
import { fetchCameras, updateAmcConfig, importAmcJsonCalibration } from '../api';
...
export default function Settings() {
  ...
  const handleSaveAmcConfig = async () => {
    ...
    try {
      setAmcStatus('Saving and uploading files...');
      await updateAmcConfig(amcFiles);
      setAmcStatus(`Successfully uploaded and configured ${amcFiles.length} cameras.`);
      // Refresh the camera list dynamically to show the new ones
      fetchCameras().then(setCameras).catch(console.error);
    } catch (err) {
  ...
  const handleUploadCalibration = async () => {
    if (!calibFile) {
      setCalibStatus('Please select a calibration.json file.');
      return;
    }
    try {
      setCalibStatus('Uploading calibration...');
      const res = await importAmcJsonCalibration(calibFile);
      if (res.success) {
        setCalibStatus(`Success: Applied calibration to ${res.updated_count} cameras.`);
        // Refresh cameras to show updated matrices if needed (though UI might not show them directly)
        fetchCameras().then(setCameras).catch(console.error);
      } else {
        setCalibStatus(`Error: ${res.error || 'Unknown error'}`);
      }
    } catch (err) {
      ...
      <div className="flex-1 p-6">
        {selectedCam === 'amc_config' ? (
          <div>
             ...
             <section className="bg-gray-50 p-6 rounded border border-gray-200 mt-6">
                <h3 className="text-lg font-semibold mb-2 text-gray-800">AMC Calibration JSON</h3>
                <p className="text-gray-600 mb-4">
                  Upload the <code>calibration.json</code> file generated by AMC. This will automatically map homography matrices to cameras based on their filenames.
                </p>
                <input 
                  type="file" 
                  accept=".json" 
                  onChange={(e) => setCalibFile(e.target.files[0])} 
                  className="mb-4 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                />
                <button 
                ...
                {calibStatus && (
                  <p className={`mt-3 text-sm ${calibStatus.includes('Error') ? 'text-red-600' : 'text-green-600'}`}>
                    {calibStatus}
                  </p>
                )}
             </section>
