...
try:
    ...
    original_getPerspectiveTransform = cv2.getPerspectiveTransform

    def hooked_getPerspectiveTransform(*args, **kwargs):
        print(">>> cv2.getPerspectiveTransform called!")
        res = original_getPerspectiveTransform(*args, **kwargs)
        if res is None:
            print("  WARNING: returned None")
        return res

    cv2.getPerspectiveTransform = hooked_getPerspectiveTransform
